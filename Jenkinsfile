      ...
      stage('helm Manifest Update') {
        steps {
          ...
          sshagent(credentials: ['git-ssh-credential']) {
            sh "helm template micro-service . --namespace micro-cicd --set images.tag=${env.BUILD_NUMBER} > ./kubernetes-manifests/kubernetes.yaml"
            sh "git add kubernetes-manifests/kubernetes.yaml"
